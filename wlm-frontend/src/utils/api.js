const URL_BASE = process.env.REACT_APP_API_BASE_URL || "https://dc-welovemovies-backend.herokuapp.com"
...
function populateReviews(signal) {
  return async (movie) => {
    const url = `${URL_BASE}/movies/${movie.movie_id}/reviews`;
...
function populateTheaters(signal) {
  return async (movie) => {
    const url = `${URL_BASE}/movies/${movie.movie_id}/theaters`;
...
export async function listMovies(signal) {
  const url = new URL(`${URL_BASE}/movies?is_showing=true`);
...
export async function listTheaters(signal) {
  const url = new URL(`${URL_BASE}/theaters`);
...
export async function readMovie(movieId, signal) {
  const url = new URL(`${URL_BASE}/movies/${movieId}`);
  const addReviews = populateReviews(signal);
...
export async function deleteReview(reviewId) {
  const url = `${URL_BASE}/reviews/${reviewId}`;
